...
const comp = (op: "=" | ">" | ">=" | "<" | "<=", ver: SemVerClass) => new Comparator({ operator: op, version: ver });

const range = (...sets: ReadonlyArray<ReadonlyArray<ComparatorClass>>) => new Range({ sets: sets.map((s) => [...s]) });

const r = (input: string) => Effect.runSync(parseRange(input));
...
describe("grammar branches", () => {
	...
	it("range rejects ~> (Ruby style)", () => {
		expect(() => Effect.runSync(parseRange("~>1.0.0"))).toThrow();
	});

	// Range with trailing content
	it("range rejects trailing content", () => {
		expect(() => Effect.runSync(parseRange("1.0.0 invalid!"))).toThrow();
	});
});
