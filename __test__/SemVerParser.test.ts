...
import { parseSingleComparator, parseValidSemVer } from "../src/utils/grammar.js";
...
describe("Convenience parsing functions (no Layer needed)", () => {
	it("parseVersion returns a SemVer", () => {
		const result = Effect.runSync(parseValidSemVer("1.2.3"));
		expect(result.major).toBe(1);
		expect(result.minor).toBe(2);
		expect(result.patch).toBe(3);
	});
	...
	it("parseVersion fails on invalid input", () => {
		expect(() => Effect.runSync(parseValidSemVer("bad"))).toThrow(/Invalid version string/);
	});
	...
	it("parseComparator returns a Comparator", () => {
		const result = Effect.runSync(parseSingleComparator(">=1.2.3"));
		expect(result.operator).toBe(">=");
		expect(result.version.major).toBe(1);
	});

	it("parseComparator fails on invalid input", () => {
		expect(() => Effect.runSync(parseSingleComparator(">>1"))).toThrow(/Invalid comparator/);
	});
});
